...
WEEX_CONTRACT="https://api-contract.weex.com"

# Use Python urllib instead of curl for cross-platform compatibility
fetch() {
  $PYTHON -c "
import sys
from urllib.request import urlopen, Request
try:
    r = urlopen(Request(sys.argv[1], headers={'User-Agent':'crypto-tracker/2.0'}), timeout=15)
...
case "$cmd" in
  ...
  price)
    ...
    ids=$(echo "$ids" | tr ' ' ',')
    fetch "$GECKO/simple/price?ids=$ids&vs_currencies=usd&include_24hr_change=true&include_market_cap=true" | $PYTHON -c '
import sys, json
...
"
  ...
  trending)
    fetch "$GECKO/search/trending" | $PYTHON -c "
import sys, json
d = json.load(sys.stdin)
...
"
  ...
  defi)
    fetch "$LLAMA/protocols" | $PYTHON -c "
import sys, json
...
"
  ...
  memes)
    fetch "$GECKO/coins/markets?vs_currency=usd&category=meme-token&order=volume_desc&per_page=15&page=1" | $PYTHON -c "
import sys, json
...
"
  ...
  info)
    ...
    coin="${1:-bitcoin}"
    fetch "$GECKO/coins/$coin" | $PYTHON -c "
import sys, json
d = json.load(sys.stdin)
name = d.get('name', '?')
...
PYEOF
  ...
  weex)
    # WEEX spot 24hr ticker
    SYMBOLS="${*:-BTCUSDT ETHUSDT SOLUSDT}"
    export _WEEX_SYMBOLS="$SYMBOLS"
    $PYTHON << 'PYEOF'
import json, os, sys
from urllib.request import urlopen, Request
symbols = os.environ.get("_WEEX_SYMBOLS", "BTCUSDT ETHUSDT SOLUSDT").split()
base = "https://api-spot.weex.com"
print("📊 WEEX Spot Market")
print("=" * 65)
print("{:<10} {:>12} {:>9} {:>12} {:>12} {:>10}".format("Symbol","Price","24h","High","Low","Vol"))
print("-" * 65)
for sym in symbols:
    s = sym.strip().upper()
    try:
        url = "{}/api/v3/market/ticker/24hr?symbol={}".format(base, s)
        r = urlopen(Request(url, headers={"User-Agent":"crypto-tracker/2.0"}), timeout=15)
        d = json.loads(r.read().decode())
        price = float(d.get("lastPrice", 0))
        pct = float(d.get("priceChangePercent", 0)) * 100
        ...
        h = "${:,.2f}".format(high)
        l = "${:,.2f}".format(low)
        v = "${:.1f}M".format(vol/1e6) if vol < 1e9 else "${:.2f}B".format(vol/1e9)
        print("{:<10} {:>12} {}{:>+.2f}% {:>12} {:>12} {:>10}".format(s, p, emoji, pct, h, l, v))
    except Exception as e:
        print("{:<10} {:>12}   {}".format(s, "ERROR", str(e)[:40]))
print()
print("Data source: WEEX Exchange (api-spot.weex.com)")
PYEOF
    ;;

  weex-futures)
    SYMBOLS="${*:-BTCUSDT ETHUSDT SOLUSDT}"
    export _WEEX_FSYMBOLS="$SYMBOLS"
    $PYTHON << 'PYEOF'
import json, os, sys
from urllib.request import urlopen, Request
symbols = os.environ.get("_WEEX_FSYMBOLS", "BTCUSDT ETHUSDT SOLUSDT").split()
base = "https://api-contract.weex.com"
print("📊 WEEX Futures Market")
print("=" * 55)
print("{:<10} {:>12} {:>12} {:>10}".format("Symbol","Price","Mark Price","Funding"))
print("-" * 55)
for sym in symbols:
    s = sym.strip().upper()
    try:
        url = "{}/capi/v3/market/symbolPrice?symbol={}".format(base, s)
        r = urlopen(Request(url, headers={"User-Agent":"crypto-tracker/2.0"}), timeout=15)
        d = json.loads(r.read().decode())
        price = float(d.get("price", 0))
        url2 = "{}/capi/v3/market/fundingRate?symbol={}".format(base, s)
        r2 = urlopen(Request(url2, headers={"User-Agent":"crypto-tracker/2.0"}), timeout=15)
        fd = json.loads(r2.read().decode())
        fr, mark = 0, 0
        if isinstance(fd, list) and fd:
            fr = float(fd[0].get("fundingRate", 0))
            mark = float(fd[0].get("markPrice", 0))
        p = "${:,.2f}".format(price)
        m = "${:,.2f}".format(mark)
        f_str = "{:+.4f}%".format(fr * 100)
        print("{:<10} {:>12} {:>12} {:>10}".format(s, p, m, f_str))
    except Exception as e:
        print("{:<10} {:>12}   {}".format(s, "ERROR", str(e)[:40]))
print()
print("Data source: WEEX Exchange (api-contract.weex.com)")
PYEOF
    ;;

  weex-funding)
    SYM="${1:-BTCUSDT}"
    export _WEEX_FSYM="$SYM"
    $PYTHON << 'PYEOF'
import json, os, sys, datetime
from urllib.request import urlopen, Request
sym = os.environ.get("_WEEX_FSYM", "BTCUSDT").strip().upper()
base = "https://api-contract.weex.com"
url = "{}/capi/v3/market/fundingRate?symbol={}".format(base, sym)
try:
    r = urlopen(Request(url, headers={"User-Agent":"crypto-tracker/2.0"}), timeout=15)
    data = json.loads(r.read().decode())
except Exception as e:
    print("API error: {}".format(e)); sys.exit(1)
if not isinstance(data, list) or not data:
    print("No funding rate data."); sys.exit(1)
print("📊 WEEX Funding Rate: {}".format(sym))
print("=" * 55)
print("  {:<18} {:>12} {:>14}".format("Time","Rate","Mark Price"))
print("  " + "-" * 50)
for item in data[:10]:
    ...
    print("  {:<18} {}{:>+.4f}% {:>14}".format(dt, emoji, rate*100, "${:,.1f}".format(mark)))
rates = [float(x.get("fundingRate", 0)) for x in data[:24]]
avg = sum(rates)/len(rates) if rates else 0
annual = avg * 3 * 365 * 100
print()
print("  Avg (recent {} periods): {:+.4f}%".format(len(rates), avg*100))
print("  Annualized: {:+.2f}%".format(annual))
print()
print("Data source: WEEX Exchange (api-contract.weex.com)")
PYEOF
    ;;
  ...
  weex-depth)
    SYM="${1:-BTCUSDT}"
    export _WEEX_DSYM="$SYM"
    $PYTHON << 'PYEOF'
import json, os, sys
from urllib.request import urlopen, Request
sym = os.environ.get("_WEEX_DSYM", "BTCUSDT").strip().upper()
base = "https://api-spot.weex.com"
url = "{}/api/v3/market/depth?symbol={}".format(base, sym)
try:
    r = urlopen(Request(url, headers={"User-Agent":"crypto-tracker/2.0"}), timeout=15)
    data = json.loads(r.read().decode())
except Exception as e:
...
print("📊 WEEX Order Book: {}".format(sym))
print("=" * 40)
print("  {:>14} {:>12}".format("Ask Price","Qty"))
print("  " + "-" * 30)
for ask in reversed(asks):
    p, q = float(ask[0]), float(ask[1])
    print("  🔴 ${:>12,.2f} {:>12.4f}".format(p, q))
print("  {:^30}".format("--- spread ---"))
for bid in bids:
    p, q = float(bid[0]), float(bid[1])
    print("  🟢 ${:>12,.2f} {:>12.4f}".format(p, q))
print("  " + "-" * 30)
print("  {:>14} {:>12}".format("Bid Price","Qty"))
print()
print("Data source: WEEX Exchange (api-spot.weex.com)")
PYEOF
    ;;
  ...
  help|*)
    echo "🪙 Crypto Tracker — Free crypto market data + WEEX Exchange"
    echo ""
    ...
    echo ""
    echo "  🔶 WEEX Exchange:"
    echo "  weex [symbols...]       Spot prices (default: BTC ETH SOL)"
    echo "  weex-futures [symbols.] Futures prices + funding rate"
    echo "  weex-funding [symbol]   Funding rate history"
    echo "  weex-depth [symbol]     Order book depth"
    echo ""
